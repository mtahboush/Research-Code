...
echo ""| awk 'BEGIN{printf "Simulation_Number,No_of_Packets_Sent,No_of_Packets_Received,Packet_Delivery_Ratio,Packet_Dropping_Ratio,Throughput,Normalized_Routing_Overhead,Control_Overhead,Delay,Jitter,Consumed_Energy,Route_Loss"}'>Output/Results/Final_Result.csv

#create 10 CBR files
ns cbrgen.tcl -type cbr -nn 75 -seed 1.0 -mc 70 -rate 6.0 > session

#Main loop which runs 10 times.
while : ; do
	((i++))
		#Creates 10 mobility files. Each of which is named as mob1, mob2, ... , mob10.
		./setdest -v 2 -n 75 -m 1 -M 20 -t 500 -p 2 -x 800 -y 800 > mobility_files/mob$i

		#Call TCL script with mobility file as a parameter.
		ns aodvwithBlachHOle.tcl mobility_files/mob$i
		...
		grep "Consumed_Energy:" tmp_result |  awk 'BEGIN{}{print $2;}' > 1.txt
		ene=$(<1.txt)

		grep "Route_Loss:" tmp_result |  awk 'BEGIN{}{print $2;}' > 1.txt
		rlts=$(<1.txt)



		#This condition handles if Packet Delivery Ratio is below 1. Sometimes if nodes are apart from each other, PDR is below 1.
		if [ 1 -eq "$(echo "${pdr} < 1" | bc)" ]
		then
			((i--))
		else
			...
			avg_energy=$( bc <<< "$ene + $avg_energy ")
			avg_routeloss=$( bc <<< "$rlts + $avg_routeloss ")
			...
			echo  "$ene" | awk '{printf ", %.5f", $1}' >> Output/Results/Final_Result.csv
			echo  "$rlts" | awk '{printf ", %.5f", $1}' >> Output/Results/Final_Result.csv	
		fi
...
echo  "$avg_energy $count" | awk '{printf ", %.5f", $1/$2}' >> Output/Results/Final_Result.csv
echo  "$avg_routeloss $count" | awk '{printf ", %.5f", $1/$2}' >> Output/Results/Final_Result.csv
